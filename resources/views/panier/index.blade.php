                            ...
                            <div class="p-6 flex flex-col md:flex-row gap-6 hover:bg-gray-50 transition">
                                ...
                                <div class="w-full md:w-1/5 lg:w-1/6">
                                    <div class="aspect-w-1 aspect-h-1 bg-gray-100 rounded-lg overflow-hidden">
                                        <img loading="lazy" src="{{ asset('storage/' . $item['image']) }}" alt="{{ $item['libelle'] }}"
                                ...
                                <div class="flex-1">
                                    <div class="flex justify-between">
                                        <div>
                                            ...
                                            <p class="text-blue-600 font-medium mt-1">{{ number_format($item['prix'], 2) }}
                                                MAD</p>
                                        </div>
                                                ...
                                                <form action="{{ route('panier.changerCouleur') }}" method="POST">
                                                    ...
                                                    <select name="nouvelle_couleur" onchange="this.form.submit()"
                                                        ...
                                                        @foreach ($item['couleurArticle'] as $index => $couleur)
                                                            <option value="{{ $item['couleurArticleIDS'][$index] }}">
                                                                {{ $couleur }}</option>
                                                        @endforeach
                                                    </select>
                                                    <input type="hidden" name="taille_id"
                                                        value="{{ $item['taille_id'] }}">
                                                ...
                                                <form action="{{ route('panier.changerTaille') }}" method="POST">
                                                    ...
                                                    <select name="nouvelle_taille" onchange="this.form.submit()"
                                                        ...
                                                        @foreach ($item['tailleArticle'] as $index => $taille)
                                                            <option value="{{ $item['tailleArticleIDS'][$index] }}">
                                                                {{ $taille }}</option>
                                                        @endforeach
                                                    </select>
                                                    <input type="hidden" name="couleur_id"
                                                        value="{{ $item['couleur_id'] }}">
                                                    <input type="hidden" name="article_id"
                                                        value="{{ $item['article_id'] }}">
                                                    <input type="hidden" name="old_key" value="{{ $id }}">
                                                </form>
                                    ...
                                    <div class="mt-6 flex flex-wrap items-center justify-between gap-4">
                                        <form action="{{ route('panier.article.update', $id) }}" method="POST"
                            ...
                            <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200 w-full md:w-1/3">
                                ...
                                <div class="space-y-3">
                                    ...
                                    <div class="flex justify-between border-t border-gray-200 pt-3 mt-3">
                                        ...
                                        <span class="text-lg font-bold text-blue-600">{{ number_format($total, 2) }}
                                            MAD</span>
                                    </div>
                                </div>
                                ...
                                <div class="mt-6 space-y-3">
                                    ...
                                    <form action="{{ route('panier.vider') }}" method="POST"
                                        ...
                                        @method('DELETE')
                                        <button type="submit"
                                            class="block w-full bg-red-500 hover:bg-red-600 text-white text-center py-3 px-4 rounded-lg shadow-md transition font-medium">
                                            Vider le panier
                                        </button>
                                    </form>
