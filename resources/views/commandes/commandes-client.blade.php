...
<body class="font-sans antialiased bg-gray-400">
    <nav class="bg-black shadow-sm border-b border-gray-200 fixed top-0 left-0 right-0 z-50 mb-10"
        x-data="{ open: false }">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div x-show="open" @click.away="open = false" class="md:hidden bg-black pb-3 px-2">
                <div class="flex flex-col space-y-3">
                    <a href="{{ route('panier.index') }}"
                        class="flex items-center text-sm font-medium text-white hover:text-blue-600 px-3 py-2">
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit"
                            class="w-full flex items-center text-sm font-medium text-white hover:text-blue-600 px-3 py-2">
                        ...
                        <ul class="space-y-2">
                            <li>
                                <a href="{{ route('profile.edit') }}"
                                    class="flex items-center space-x-2 px-3 py-2 text-gray-700 hover:bg-gray-100 rounded-lg">
                                    <i class="bi bi-person-gear"></i>
                                    <span>Mon profil</span>
                                </a>
                            </li>
                            <li>
                                <a href="{{ route('commandes.client', Auth::id()) }}"
                                    class="flex items-center space-x-2 px-3 py-2 bg-blue-50 text-blue-600 rounded-lg font-medium">
                ...
                <div class="bg-white rounded-lg shadow-sm overflow-hidden">
                    ...
                    @if ($commandes->isEmpty())
                        <div class="p-8 text-center">
                            ...
                            @if (!Auth::check())
                                <a href="{{ url('/') }}"
                                    class="inline-flex items-center px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg shadow-md transition">
                                    ...
                                    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-4">
                                        <div class="mb-4 md:mb-0">
                                            <div class="flex items-center space-x-4">
                                                <h3 class="text-lg font-medium text-gray-900">Commande
                                                    #{{ $commande->id }}</h3>
                                                <span
                                            ...
                                            <p class="text-sm text-gray-500 mt-1">Passée le
                                                {{ $commande->created_at->format('d/m/Y à H:i') }}</p>
                                        </div>
                                        <div class="flex items-center space-x-4">
                                            <span
                                                class="text-lg font-semibold">{{ number_format($commande->total, 2) }}
                                                MAD</span>
                                            ...
                                            <div class="p-4 flex items-start border-b border-gray-200 last:border-b-0">
                                                ...
                                                <div class="ml-4 flex-1">
                                                    ...
                                                    <div class="mt-2 flex items-center justify-between">
                                                        <span class="text-sm text-gray-900">Quantité:
                                                            {{ $ligne->quantite_commande }}</span>
                                                        <span
                                                            class="text-sm font-medium">{{ number_format($ligne->prix * $ligne->quantite_commande, 2) }}
                                                            MAD</span>
                                                    </div>
                                                </div>
                                            </div>
